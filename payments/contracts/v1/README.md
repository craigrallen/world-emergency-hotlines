...
- **Webhook verification.** `/billing/api/webhook` verifies `Stripe-Signature` (HMAC-SHA256 over `timestamp.body`, 300-second tolerance, constant-time compare), rejects events whose `livemode` disagrees with the deployment mode, deduplicates by event id (a claim counts as a duplicate only once the event was applied; an incomplete claim is retried, and one abandoned by a dead worker is taken over after 120 seconds), and records only pseudonymous identifiers plus closed enum statuses. Ordering is per event family by `created`; after a family's first write, accepted deliveries retrieve current Stripe state, including deliveries from newer seconds; historical timestamps cannot version a fetched snapshot (a retrieval failure fails the delivery so Stripe retries it).
- **Customer Portal.** Only authenticated CMS account ownership authorizes `/cms/api/account/portal`. The anonymous `/billing/api/portal-session` route has been removed (404); checkout identifiers are not authorization and are never appended to success URLs. Unlinked purchases require maintainer-assisted ownership verification.
