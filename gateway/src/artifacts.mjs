...
const MANAGED_RELATIONSHIP_PATHS=RELATIONSHIP_PATHS.filter(path=>!path.startsWith('/technical-health/v1/'));
const MANAGED_BUILD_INPUTS={...BUILD_INPUTS,integration_generator:BUILD_INPUTS.integration_generator.filter(path=>path!=='scripts/generate-technical-health-contracts.mjs'&&path!=='repo:technical-health/model.mjs')};
...
function coveredArtifactPath(path,profile){return path.startsWith('/data/')||path.startsWith('/api/v1/')||path==='/widget/v1/hotlines-widget.js'||path==='/release/v1/changes.json'||path.startsWith('/release/v1/changes/')||path.startsWith('/feeds/')||path.startsWith('/subscriptions/v1/')||path.startsWith('/gateway/v1/')||(profile!=='legacy'&&path.startsWith('/organizations/v1/'))||(['managed','current'].includes(profile)&&path.startsWith('/managed-widget-config/v1/'))||(profile==='current'&&path.startsWith('/technical-health/v1/'));}
