...
function coveredArtifactPath(path,profile){return path.startsWith('/data/')||path.startsWith('/api/v1/')||path==='/widget/v1/hotlines-widget.js'||path==='/release/v1/changes.json'||path.startsWith('/release/v1/changes/')||path.startsWith('/feeds/')||path.startsWith('/subscriptions/v1/')||path.startsWith('/gateway/v1/')||(profile!=='legacy'&&path.startsWith('/organizations/v1/'))||(['managed','technical','assurance','current'].includes(profile)&&path.startsWith('/managed-widget-config/v1/'))||(['technical','assurance','current'].includes(profile)&&path.startsWith('/technical-health/v1/'))||(['assurance','current'].includes(profile)&&path.startsWith('/assurance-packs/v1/'))||(profile==='current'&&path.startsWith('/provider-claims/v1/'));}
function validateTrustedRelease(release,index){
  ...
  const ai=release.artifact_index;if(!exactObject(ai,['path','sha256','artifact_count','coverage','excludes'])||ai.path!=='/release/v1/artifacts.json'||!SHA256_ID.test(ai.sha256)||ai.artifact_count!==index.artifacts.length||!sameJson(ai.coverage,profile==='current'?COVERAGE:profile==='assurance'?ASSURANCE_COVERAGE:profile==='technical'?TECHNICAL_COVERAGE:profile==='managed'?MANAGED_COVERAGE:profile==='organization'?ORGANIZATION_COVERAGE:LEGACY_COVERAGE)||!sameJson(ai.excludes,EXCLUDES))throw new Error('invalid trusted release input');
