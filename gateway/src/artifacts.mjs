...
const COVERAGE=['/data/**','/api/v1/**','/widget/v1/hotlines-widget.js','/release/v1/changes.json','/release/v1/changes/**','/feeds/**','/subscriptions/v1/**','/gateway/v1/**','/organizations/v1/**','/managed-widget-config/v1/**','/technical-health/v1/**','/assurance-packs/v1/**','/provider-claims/v1/**','/reviewer-work-queue/v1/**','/managed-api-plans/v1/**'];
const REVIEWER_COVERAGE=COVERAGE.slice(0,-1);
...
const REVIEWER_BUILD_INPUTS={...BUILD_INPUTS,integration_generator:BUILD_INPUTS.integration_generator.filter(path=>path!=='scripts/generate-managed-api-plan-contracts.mjs'&&path!=='repo:managed-api-plans/model.mjs')};
const PROVIDER_RELATIONSHIP_PATHS=RELATIONSHIP_PATHS.filter(path=>!path.startsWith('/reviewer-work-queue/v1/'));
...
function coveredArtifactPath(path,profile){return path.startsWith('/data/')||path.startsWith('/api/v1/')||path==='/widget/v1/hotlines-widget.js'||path==='/release/v1/changes.json'||path.startsWith('/release/v1/changes/')||path.startsWith('/feeds/')||path.startsWith('/subscriptions/v1/')||path.startsWith('/gateway/v1/')||(profile!=='legacy'&&path.startsWith('/organizations/v1/'))||(['managed','technical','assurance','provider','reviewer','current'].includes(profile)&&path.startsWith('/managed-widget-config/v1/'))||(['technical','assurance','provider','reviewer','current'].includes(profile)&&path.startsWith('/technical-health/v1/'))||(['assurance','provider','reviewer','current'].includes(profile)&&path.startsWith('/assurance-packs/v1/'))||(['provider','reviewer','current'].includes(profile)&&path.startsWith('/provider-claims/v1/'))||(['reviewer','current'].includes(profile)&&path.startsWith('/reviewer-work-queue/v1/'))||(profile==='current'&&path.startsWith('/managed-api-plans/v1/'));}
function validateTrustedRelease(release,index){
  ...
  const ai=release.artifact_index;if(!exactObject(ai,['path','sha256','artifact_count','coverage','excludes'])||ai.path!=='/release/v1/artifacts.json'||!SHA256_ID.test(ai.sha256)||ai.artifact_count!==index.artifacts.length||!sameJson(ai.coverage,profile==='current'?COVERAGE:profile==='reviewer'?REVIEWER_COVERAGE:profile==='provider'?PROVIDER_COVERAGE:profile==='assurance'?ASSURANCE_COVERAGE:profile==='technical'?TECHNICAL_COVERAGE:profile==='managed'?MANAGED_COVERAGE:profile==='organization'?ORGANIZATION_COVERAGE:LEGACY_COVERAGE)||!sameJson(ai.excludes,EXCLUDES))throw new Error('invalid trusted release input');
