...
const COVERAGE=['/data/**','/api/v1/**','/widget/v1/hotlines-widget.js','/release/v1/changes.json','/release/v1/changes/**','/feeds/**','/subscriptions/v1/**','/gateway/v1/**','/organizations/v1/**'];
const LEGACY_COVERAGE=COVERAGE.slice(0,-1);
const EXCLUDES=['/release/v1/artifacts.json','/release/v1/release.json'];
const RELATIONSHIP_PATHS=['/data/manifest.json','/api/v1/manifest.json','/api/v1/records.json','/api/v1/resolver.js','/widget/v1/hotlines-widget.js','/data/metadata-coverage.json','/data/categories-stats.json','/data/search-index.json','/release/v1/changes.json','/release/v1/changes/latest.json','/feeds/releases.json','/feeds/releases.rss','/feeds/releases.atom','/subscriptions/v1/README.md','/subscriptions/v1/common.schema.json','/subscriptions/v1/event.schema.json','/subscriptions/v1/subscription-request.schema.json','/subscriptions/v1/subscription-response.schema.json','/subscriptions/v1/error.schema.json','/subscriptions/v1/openapi.json','/subscriptions/v1/webhook-contract.json','/subscriptions/v1/fixture-baseline.json','/subscriptions/v1/fixture-no-change.json','/subscriptions/v1/fixture-added.json','/subscriptions/v1/fixture-modified.json','/subscriptions/v1/fixture-country-metadata.json','/gateway/v1/README.md','/gateway/v1/artifact-descriptor.schema.json','/gateway/v1/error.schema.json','/gateway/v1/health.schema.json','/gateway/v1/key-record.schema.json','/gateway/v1/openapi.json','/gateway/v1/privacy.json','/gateway/v1/security.json','/organizations/v1/README.md','/organizations/v1/fixture.synthetic.json','/organizations/v1/model.schema.json','/organizations/v1/openapi.json'];
const BUILD_INPUTS={integration_generator:['scripts/build-static-data.mjs','scripts/centroids.json','scripts/dataset-diff.mjs','scripts/generate-gateway-contracts.mjs','scripts/generate-organization-contracts.mjs','repo:control-plane/model.mjs','scripts/generate-subscription-contracts.mjs','scripts/metadata-coverage.mjs','scripts/release-feeds.mjs','scripts/release-integrity.mjs','scripts/subscription-events.mjs'],resolver_code:['src/lib/finder.js'],widget_code:['public/widget/v1/hotlines-widget.js']};
const LEGACY_RELATIONSHIP_PATHS=RELATIONSHIP_PATHS.filter(path=>!path.startsWith('/organizations/v1/'));
const LEGACY_BUILD_INPUTS={...BUILD_INPUTS,integration_generator:BUILD_INPUTS.integration_generator.filter(path=>path!=='scripts/generate-organization-contracts.mjs'&&path!=='repo:control-plane/model.mjs')};
...
function coveredArtifactPath(path,extended){return path.startsWith('/data/')||path.startsWith('/api/v1/')||path==='/widget/v1/hotlines-widget.js'||path==='/release/v1/changes.json'||path.startsWith('/release/v1/changes/')||path.startsWith('/feeds/')||path.startsWith('/subscriptions/v1/')||path.startsWith('/gateway/v1/')||(extended&&path.startsWith('/organizations/v1/'));}
function validateTrustedRelease(release,index){
  ...
  const extended=sameJson(release?.artifact_index?.coverage,COVERAGE),legacy=sameJson(release?.artifact_index?.coverage,LEGACY_COVERAGE);if(!extended&&!legacy)throw new Error('invalid trusted release input');const relationshipPaths=extended?RELATIONSHIP_PATHS:LEGACY_RELATIONSHIP_PATHS,buildInputs=extended?BUILD_INPUTS:LEGACY_BUILD_INPUTS;
  ...
  let previous='';const byPath=new Map();for(const entry of index.artifacts){if(!artifactEntry(entry)||!coveredArtifactPath(entry.path,extended)||EXCLUDES.includes(entry.path)||entry.path<=previous||byPath.has(entry.path))throw new Error('invalid trusted release input');previous=entry.path;byPath.set(entry.path,entry);}
  ...
  const ai=release.artifact_index;if(!exactObject(ai,['path','sha256','artifact_count','coverage','excludes'])||ai.path!=='/release/v1/artifacts.json'||!SHA256_ID.test(ai.sha256)||ai.artifact_count!==index.artifacts.length||!sameJson(ai.coverage,extended?COVERAGE:LEGACY_COVERAGE)||!sameJson(ai.excludes,EXCLUDES))throw new Error('invalid trusted release input');
