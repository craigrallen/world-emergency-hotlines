...
const COVERAGE=['/data/**','/api/v1/**','/widget/v1/hotlines-widget.js','/release/v1/changes.json','/release/v1/changes/**','/feeds/**','/subscriptions/v1/**','/gateway/v1/**','/organizations/v1/**','/managed-widget-config/v1/**','/technical-health/v1/**','/assurance-packs/v1/**','/provider-claims/v1/**','/reviewer-work-queue/v1/**','/managed-api-plans/v1/**','/deprecation-proposals/v1/**','/evidence-backed-coverage/v1/**'];
...
function coveredArtifactPath(path,profile){return path.startsWith('/data/')||path.startsWith('/api/v1/')||path==='/widget/v1/hotlines-widget.js'||path==='/release/v1/changes.json'||path.startsWith('/release/v1/changes/')||path.startsWith('/feeds/')||path.startsWith('/subscriptions/v1/')||path.startsWith('/gateway/v1/')||(profile!=='legacy'&&path.startsWith('/organizations/v1/'))||(['managed','technical','assurance','provider','reviewer','plan','evidence','current'].includes(profile)&&path.startsWith('/managed-widget-config/v1/'))||(['technical','assurance','provider','reviewer','plan','evidence','current'].includes(profile)&&path.startsWith('/technical-health/v1/'))||(['assurance','provider','reviewer','plan','evidence','current'].includes(profile)&&path.startsWith('/assurance-packs/v1/'))||(['provider','reviewer','plan','evidence','current'].includes(profile)&&path.startsWith('/provider-claims/v1/'))||(['reviewer','plan','evidence','current'].includes(profile)&&path.startsWith('/reviewer-work-queue/v1/'))||(['plan','evidence','current'].includes(profile)&&path.startsWith('/managed-api-plans/v1/'))||(['evidence','current'].includes(profile)&&path.startsWith('/deprecation-proposals/v1/'))||(profile==='current'&&path.startsWith('/evidence-backed-coverage/v1/'));}
function validateTrustedRelease(release,index){
  ...
  const profile=sameJson(release?.artifact_index?.coverage,COVERAGE)?'current':sameJson(release?.artifact_index?.coverage,EVIDENCE_COVERAGE)?'evidence':sameJson(release?.artifact_index?.coverage,PLAN_COVERAGE)?'plan':sameJson(release?.artifact_index?.coverage,REVIEWER_COVERAGE)?'reviewer':sameJson(release?.artifact_index?.coverage,PROVIDER_COVERAGE)?'provider':sameJson(release?.artifact_index?.coverage,ASSURANCE_COVERAGE)?'assurance':sameJson(release?.artifact_index?.coverage,TECHNICAL_COVERAGE)?'technical':sameJson(release?.artifact_index?.coverage,MANAGED_COVERAGE)?'managed':sameJson(release?.artifact_index?.coverage,ORGANIZATION_COVERAGE)?'organization':sameJson(release?.artifact_index?.coverage,LEGACY_COVERAGE)?'legacy':null;if(!profile)throw new Error('invalid trusted release input');const relationshipPaths=profile==='current'?RELATIONSHIP_PATHS:profile==='evidence'?EVIDENCE_RELATIONSHIP_PATHS:profile==='plan'?PLAN_RELATIONSHIP_PATHS:profile==='reviewer'?REVIEWER_RELATIONSHIP_PATHS:profile==='provider'?PROVIDER_RELATIONSHIP_PATHS:profile==='assurance'?ASSURANCE_RELATIONSHIP_PATHS:profile==='technical'?TECHNICAL_RELATIONSHIP_PATHS:profile==='managed'?MANAGED_RELATIONSHIP_PATHS:profile==='organization'?ORGANIZATION_RELATIONSHIP_PATHS:LEGACY_RELATIONSHIP_PATHS,buildInputs=profile==='current'?BUILD_INPUTS:profile==='evidence'?EVIDENCE_BUILD_INPUTS:profile==='plan'?PLAN_BUILD_INPUTS:profile==='reviewer'?REVIEWER_BUILD_INPUTS:profile==='provider'?PROVIDER_BUILD_INPUTS:profile==='assurance'?ASSURANCE_BUILD_INPUTS:profile==='technical'?TECHNICAL_BUILD_INPUTS:profile==='managed'?MANAGED_BUILD_INPUTS:profile==='organization'?ORGANIZATION_BUILD_INPUTS:LEGACY_BUILD_INPUTS;
  ...
  let previous='';const byPath=new Map();for(const entry of index.artifacts){if(!artifactEntry(entry)||!coveredArtifactPath(entry.path,profile)||EXCLUDES.includes(entry.path)||entry.path<=previous||byPath.has(entry.path))throw new Error('invalid trusted release input');previous=entry.path;byPath.set(entry.path,entry);}
  ...
  if(!exactObject(release.build_versions,['integration_generator','resolver_code','widget_code'])||!Object.values(release.build_versions).every(v=>SHA256_ID.test(v))||!exactObject(release.build_version_semantics,['algorithm','inputs'])||release.build_version_semantics.algorithm!==BUILD_SEMANTICS||!sameJson(release.build_version_semantics.inputs,buildInputs))throw new Error('invalid trusted release input');
