...
Layout: `src/env.ts` (fail-closed env parsing), `src/access.ts` (roles), `src/collections/` (users, plans, subscriptions, entitlements, stripe-events, api-keys), `src/lib/stripe.ts` (Stripe client + event handlers), `src/lib/subscriptions.ts` (idempotent subscription mirror shared by both webhook consumers), `src/lib/gateway-keys.ts` (key records in the gateway contract shape), `src/endpoints/` (`/cms/api/account/*`, `/cms/api/gateway/keys`, and the signed `/cms/api/stripe/webhooks` endpoint, which answers non-2xx on failure so Stripe retries), `src/app/` (Next.js routes: admin at `/admin`, REST at `/cms/api`). `Dockerfile` and `railway.toml` describe a separate Railway service whose root directory is this folder.
