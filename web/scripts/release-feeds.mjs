...
import { canonicalJson, codePointCompare, diffSnapshots, SHA256_RE, snapshotDataset, validateSnapshot, valueIdentity } from './dataset-diff.mjs';
...
const sortedUnique = (values, label) => { for (let i = 1; i < values.length; i++) if (codePointCompare(values[i - 1], values[i]) >= 0) throw new Error(`${label} must be strictly code-point sorted and unique`); };
...
function validateCounts(value, label) {
  closed(value, ['added', 'removed', 'modified', 'country_metadata_changed', 'total_changes'], [], label);
...
function validateChange(change, index) {
  ...
  if (own(change, 'changed_fields')) { if (!Array.isArray(change.changed_fields)) throw new Error(`${label}.changed_fields must be an array`); change.changed_fields.forEach((field) => safeText(field, `${label}.changed_fields`)); sortedUnique(change.changed_fields, `${label}.changed_fields`); }
  if (own(change, 'country_changed_fields')) { if (!Array.isArray(change.country_changed_fields)) throw new Error(`${label}.country_changed_fields must be an array`); change.country_changed_fields.forEach((field) => safeText(field, `${label}.country_changed_fields`)); sortedUnique(change.country_changed_fields, `${label}.country_changed_fields`); }
...
function validateDiff(diff, label = 'changes') {
  ...
  if (diff.schema_version !== '1.0' || diff.value_policy !== 'No before/after hotline values are published.') throw new Error(`${label} contract is invalid`);
  ...
  for (const [index, item] of diff.country_metadata_changes.entries()) { closed(item, ['country_code', 'country_name', 'previous_country_name', 'changed_fields'], [], `${label}.country_metadata_changes[${index}]`); safeText(item.country_code, 'country code'); safeText(item.country_name, 'country name'); safeText(item.previous_country_name, 'previous country name'); if (canonicalJson(item.changed_fields) !== '["country_name"]') throw new Error('country metadata changed_fields is invalid'); }
  sortedUnique(diff.country_metadata_changes.map((item) => item.country_code), `${label} metadata country codes`);
  for (const [index, item] of diff.countries.entries()) { closed(item, ['country_code', 'country_name', 'added', 'removed', 'modified', 'moved_in', 'moved_out', 'metadata_changed'], [], `${label}.countries[${index}]`); safeText(item.country_code, 'country code'); safeText(item.country_name, 'country name'); for (const key of ['added', 'removed', 'modified', 'moved_in', 'moved_out', 'metadata_changed']) count(item[key], `country summary ${key}`); }
...
export function validateRegistry(registry, currentDataset = null, exactDatasetVersion = null, { readSnapshot = (path) => readFileSync(path) } = {}) {
  ...
  if (registry.schema_version !== '1.0') throw new Error('unsupported dataset registry schema'); safeText(registry.contract, 'registry contract'); hash(registry.history_head, 'registry history_head');
...
export function orderedFeedEntries(changelog, registry) {
  ...
  return [...milestones, ...datasets].sort((a, b) => codePointCompare(b.date, a.date) || codePointCompare(a.kind, b.kind) || codePointCompare(a.source_id, b.source_id));
}
...
export function generateReleaseFeeds({ currentDataset, datasetVersion }) {
  ...
  const current = validateRegistry(registry, currentDataset, datasetVersion); const releaseRoot = resolve(PUBLIC_ROOT, 'release', 'v1'); const feedsRoot = resolve(PUBLIC_ROOT, 'feeds');
  ...
  const index = { schema_version: '1.0', canonical_origin: ORIGIN, ordering: 'Newest first by date, then stable release ID in Unicode scalar-value order.', latest: '/release/v1/changes/latest.json', releases: [...history].sort((a, b) => codePointCompare(b.date, a.date) || codePointCompare(a.id, b.id)) };
  ...
  const feeds = renderFeeds(changelog, registry); validateRenderedFeeds(feeds); writeFileSync(resolve(feedsRoot, 'releases.json'), pretty(feeds.jsonFeed)); writeFileSync(resolve(feedsRoot, 'releases.rss'), feeds.rss); writeFileSync(resolve(feedsRoot, 'releases.atom'), feeds.atom);
