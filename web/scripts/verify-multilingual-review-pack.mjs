...
import { encodePack, encodeSchema, generateCanonicalReviewPackSchema, generateReviewPack, reviewPackSafetyErrors } from './multilingual-review-pack.mjs';

const repo = resolve(import.meta.dirname, '../..');
const read = (path) => readFileSync(resolve(repo, path), 'utf8');
const pack = JSON.parse(read('reviews/multilingual-ui/v1/review-pack.json'));
...
if (encodePack(pack) !== encodePack(expected)) throw new Error('review pack is stale or differs from its canonical sources');
