...
| `/account`, `/account/verify`, `/account/reset-password` | `src/pages/account/*.astro` + `src/lib/account.js` | `noindex` static shells for the Payload CMS accounts backend (`cms/`). Every control renders disabled unless `PUBLIC_ACCOUNTS_MODE=enabled` at build time, and the client script only enables them after `GET /cms/api/account/status` answers through Caddy; sign-in, checkout, portal, and managed API keys all go to same-origin `/cms/api/*`. See `docs/ACCOUNTS.md`. |
...
- `/cms/*`, `/admin`, `/admin/*`, and `/_next/*` are reverse-proxied to the separate Payload CMS (`cms/`) only when `CMS_UPSTREAM` is set and otherwise answer `503 accounts_disabled` with `no-store`; `npm run verify:accounts`, `verify-caddy.sh`, and `verify-docker-image.sh` enforce that state.
- `/billing/api/*` is the other non-static route in the Caddyfile. It reverse-proxies to the separate `payments/` service only when `PAYMENTS_UPSTREAM` is set and otherwise answers `503 payments_disabled` with `no-store`; it never serves from disk. `npm run verify:payments`, `verify-caddy.sh`, and `verify-docker-image.sh` enforce that disabled state.
