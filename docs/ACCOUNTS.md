...
npm install
npm run dev                     # bootstrap credentials create the admin; /cms/api/account/status
...
- `/users/first-register` is disabled, including on an empty database. Configure `CMS_ADMIN_EMAIL` and `CMS_ADMIN_PASSWORD` before the first production startup. The empty production database refuses to serve without them; bootstrap creates a verified administrator. Existing administrators permit normal startup without bootstrap variables. If a database has users but no administrator, startup also refuses service unless the same explicit secret-backed variables are supplied: bootstrap resets and promotes the account matching `CMS_ADMIN_EMAIL`, or creates that admin if the address is absent. Remove the variables after recovery. Admin updates and deletes refuse to remove the last administrator. This recovery path never opens public first-register.
