...
npm install
npm run dev                     # open http://localhost:3000/admin to create the first administrator
...
Activation remains disabled by default. No account, billing or key changes gate any free crisis information, static API, widget or feed.

- Password resets preserve the database-loaded role, scope, verification state, billing links and API credentials. The forgot-password update also retains stored privileged fields. Ordinary client writes cannot set those privileges. Member/staff self-service email changes are refused until a complete pending-email verification flow exists.
- In non-production with SQLite and no `CMS_ADMIN_EMAIL`/`CMS_ADMIN_PASSWORD`, `/admin` uses Payload’s normal `/users/first-register` flow: only an empty users collection accepts it, the first user becomes a verified administrator and is logged in, and the 12–256 character password policy applies. This works with member registration `closed`; ordinary user creation does not gain admin privileges. Once any user exists, first-register refuses further setup. With Postgres, configured bootstrap credentials, or production, web first-register is disabled, including on an empty database. Configure `CMS_ADMIN_EMAIL` and `CMS_ADMIN_PASSWORD` before the first production startup. The empty production database refuses to serve without them; bootstrap creates a verified administrator. Existing administrators permit normal startup without bootstrap variables. If a database has users but no administrator, startup also refuses service unless the same explicit secret-backed variables are supplied: bootstrap resets and promotes the account matching `CMS_ADMIN_EMAIL`, or creates that admin if the address is absent. Remove the variables after recovery. Admin updates and deletes refuse to remove the last administrator. This recovery path never opens public first-register.
- Stripe event timestamps are historical delivery watermarks, **not versions of fetched snapshots**. Once a family has a stored watermark, accepted webhook updates reconcile against current Stripe state under the CMS row lock or payments revision CAS, even when the delivery comes from a newer second. Older watermarks are discarded. On CAS conflict, payments re-reads and fetches again. An unavailable current snapshot fails the webhook for retry. Mirroring unchanged family fields remains idempotent; with no CMS Stripe client the payments service is the sole authoritative writer.
- Post-watermark reconciliation performs synchronous Stripe reads on webhook activation and may read again after a payments-store CAS conflict. Provision healthy Stripe API availability and rate-limit headroom before enabling either consumer. A strictly newer invoice delivery is retained when the current subscription has no `latest_invoice`; an equal-second delivery remains ambiguous and stale. Invoice reconciliation owns only invoice fields and cannot resurrect subscription status.
- **Atomic rollout requirement:** deploy the CMS reconciliation behavior and payments-service reconciliation behavior together before activating webhooks. Do not run mixed old/new CMS and payments revisions against the shared store.
- The only billing portal entry point is authenticated `/cms/api/account/portal`, bound to the signed-in user's customer for the deployment's billing mode. Anonymous `/billing/api/portal-session` has been removed. Success URLs carry no checkout reference; Caddy additionally removes legacy `session_id` and `token` query values from access logs. Purchases without an account need maintainer-assisted ownership verification; a checkout reference alone never proves ownership.
- Payments store completion/release uses `POST /cms/api/stripe-events/lease` with `{eventId, lease, action: "complete" | "release"}` and returns `{applied: boolean}`. The service scope is required and the consumer is fixed to `payments`. Generic service DELETE and generic completion PATCH are refused; takeover PATCH remains available and rechecks expiry under a lock. Update CMS and payments together before activation. Both consumer paths lock, re-read the lease, then mutate by id in the same transaction. A stale worker cannot complete or delete a successor's claim.
- Subscription-bound exports require `subscription.user` and `key.user` to be present and equal. Reassignment and unlink therefore revoke the former owner's projected grant even while the subscription remains active.
- The CMS webhook counts bytes while reading a stream and returns 413 after 262144 bytes; it cancels the reader before retaining oversized data. Caddy enforces the same limit on the webhook before proxying, covering buffering inside Next/Payload. Keep that proxy boundary when deploying this endpoint.

### Repeatable checks

`npm run lint`, `npm run typecheck`, `npm test`, and `npm run audit:policy` run in CMS CI, with `git diff --check` over the proposed changes. The audit policy and its residual-risk exceptions are described in [CMS-DEPENDENCIES.md](CMS-DEPENDENCIES.md).

The Postgres CI job uses an ephemeral Postgres 16 service and synthetic credentials: it runs `NODE_ENV=production npm run migrate` against an empty database, builds the production app without runtime configuration, copies `.next/static` into `.next/standalone/.next/static`, and boots the same `.next/standalone/server.js` artifact used by Docker/Railway with explicit `HOSTNAME` and `PORT`. It then checks status, bootstrap login, first-user refusal and admin HTML. Finally it runs `TEST_POSTGRES_URL=postgres://… npm run test:postgres`; when `CI` is set, omission of that URL is a hard failure. This focused suite uses real transactions and observes a competing worker waiting in `pg_stat_activity` before allowing a successor transaction to commit; both CMS and the actual payments store adapter must refuse the old lease. It also tests competing takeovers and persisted ownership reassignment/unlink. Schema push is disabled for Postgres, so these checks exercise the committed migrations. Use only a disposable database for this suite; it creates synthetic records and is not an operational diagnostic for a live database.
