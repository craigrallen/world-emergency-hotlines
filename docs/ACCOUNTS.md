...
- **Webhook retries.** Both webhook endpoints answer non-2xx (500 `handler_failed`) when an event cannot be applied, after releasing the consumer's claim, so Stripe retries automatically (for days, with backoff). A Dashboard resend is only needed for an event Stripe never delivered.
