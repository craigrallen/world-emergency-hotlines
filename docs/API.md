...
1. Fetch `/release/v1/release.json` and compare its `release_id` with the descriptor your integration reviewed.
2. Verify the API manifest, resolver, widget, and required data artifacts against the descriptor/index SHA-256 values.
3. Compare `dataset_version` with the canonical-data version your integration last processed.
4. Reuse cached artifacts according to your reviewed policy and HTTP caching; periodically revalidate because `/v1/` paths are mutable.
5. When the dataset changes, fetch required country artifacts or `records.json` again. When only release/code identity changes, review and retest the changed bytes independently.

`dataset_version` identifies only the exact canonical `hotlines.json` bytes. Artifact checksums identify generated bytes, while finite-source-set build versions identify the generator/resolver/widget inputs documented on the [release integrity page](https://worldhotlines.org/release). The API and data manifests' `generated_at` fields are ISO build timestamps; the release descriptor's separate `generated_at` field is null. Neither is a dataset identity. `SOURCE_DATE_EPOCH` is required to reproduce generated artifacts byte-for-byte; without it, timestamp-bearing artifacts, their index digest, and `release_id` can change while dataset/build-source identities remain stable.

Checksums are unsigned: after obtaining a descriptor through a trusted channel they detect byte mismatch, but do not authenticate the publisher or prove freshness. On mismatch, stop promotion, keep your reviewed safe degraded state, and refetch/revalidate rather than guessing. Retain a last-reviewed artifact set for rollback only if your policy permits it.
